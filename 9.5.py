...
def summary_plot(shap_values, id, features=None, feature_names=None, max_display=None, plot_type="dot", color=None, axis_color="#333333", title=None, alpha=1, show=True, sort=True, color_bar=True, auto_size_plot=True, layered_violin_max_num_bins=20, class_names=None, xrange=None):
    ...
    if plot_type == "dot":
        for pos, i in enumerate(feature_order):
            ...
            if features is not None and colored_feature:
                ...
                pl.scatter(shaps[np.invert(nan_mask)], pos + ys[np.invert(nan_mask)],
                           cmap=red_green, vmin=vmin, vmax=vmax, s=16,
            ...
            for pos, i in enumerate(feature_order):
                ...
                if vmin == vmax:
                    ...
                    if vmin == vmax:
                        vmin = np.min(values)
                        vmax = np.max(values)
                pl.scatter(shaps, np.ones(shap_values.shape[0]) * pos, s=9, cmap=red_green, vmin=vmin, vmax=vmax,
                ...
                for i in range(len(xs) - 1):
                    if ds[i] > 0.05 or ds[i + 1] > 0.05:
                        pl.fill_between([xs[i], xs[i + 1]], [pos + ds[i], pos + ds[i + 1]],
                                        [pos - ds[i], pos - ds[i + 1]], color=red_green(smooth_values[i]),
                                        zorder=2)
        ...
        pl.barh(y_pos, global_shap_values[feature_inds], 0.7, align='center', color=color, left=np.amin(shap_values)*1.1-np.max(global_shap_values)) if xrange is None else pl.barh(y_pos, global_shap_values[feature_inds], 0.7, align='center', color=color, left=xrange[0])
    ...
    if color_bar and features is not None and plot_type != "bar" and \
        ...
        m = cm.ScalarMappable(cmap=red_green if plot_type != "layered_violin" else pl.get_cmap(color))
        m.set_array([0, 1])
...
def script():
    ...
    idx = np.random.choice(list(range(shap_values.shape[0])), size=num_rows_2_sample, replace=False)
    shap_values = shap_values[idx,:]
    X = X[idx,:]

    if type(shap_values) == list:
        shap_values = shap_values[class_val]
        title = '{0} is {1}'.format(in_data.domain.class_var.name, class_names[class_val])
    else:
        title = in_data.domain.class_var.name

    id = len(Window.get_all_windows())
    summary_plot(shap_values, features=X, feature_names=feat_names, class_names=class_names, max_display=max_display, plot_type='bar', color='black', id=id, xrange=xrange)
    summary_plot(shap_values, features=X, feature_names=feat_names, class_names=class_names, max_display=max_display, plot_type='dot', title=title, id=id, xrange=xrange)
